...
def main():
    ...
    write_to_dir(dumper, directory, work, manifestation, item)
...
def write_to_dir(dumper, directory, *records):
    for record in records:
        record_type = record.category.partition(':')[2].lower()
        file_name = f"{record_type}_{record.id}.json"
        output_path = directory / file_name
        container = efi.MovingImageRecordContainer(has_record=record)
        dumper.dump(record, to_file=output_path)
